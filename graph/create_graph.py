...
for doc in collection.find():
    user_id = doc['author']['display_name']

    if not G.has_node(user_id):
        G.add_node(user_id, type='user')

    for analysis in doc.get('analysis', []):
        medication = analysis.get('name')

        if medication == "X":
            continue

        if medication and not G.has_node(medication):
            G.add_node(medication, type='medication')

        # Connect user to medication
        if medication:
            G.add_edge(user_id, medication, relationship='posted_about')

        # Process ADRs
        for adr in analysis.get('adrs', []):
            if not G.has_node(adr):
                G.add_node(adr, type='adr')

            # Connect medication to ADR
            if medication:
                G.add_edge(medication, adr, relationship='has_adr')

            # Connect user directly to ADR
            G.add_edge(user_id, adr, relationship='experienced_adr')
...
# Improve legend
legend_elements = [
    plt.Line2D([0], [0], marker='o', color='w', label='Users', markerfacecolor='blue', markersize=8),
...
nx.write_gexf(
    G,
    "/output/medicamentos_adrs_usuarios_network.gexf",
    encoding="utf-8",
    prettyprint=True,
    version="1.2draft"
)
